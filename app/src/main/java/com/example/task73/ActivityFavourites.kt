            ...
            if (entityImages!!.isEmpty()){
                ...
                if (App.instance!!.needUpdateData){
                    ...
                    for (i in entityImages) {
                        val catImage = Json{ignoreUnknownKeys = true}.decodeFromString<Cat>(i!!.imageCat!!)
                        data.add(catImage)
                    }
                    runOnUiThread {
    ...
    private fun requestToApi() {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                val votes =
                    Json { ignoreUnknownKeys = true }.decodeFromString<List<Vote>>(responseVotes)
                ...
                for (i in votes) {
                    if (i.value == 1) {
                        ...
                        val catImage =
                            Json { ignoreUnknownKeys = true }.decodeFromString<Cat>(responseImage)
                        data.add(catImage)
